...
import (
	...
	"github.com/nekizz/final-project/backend/orchestrator/orchestrator"
...
func initMessenger(dbOrm *gorm.DB) (*kafka.Publisher, *message.Router, error) {
	...
	subscriberConfig := kafka.SubscriberConfig{
		...
		ConsumerGroup:         constant.OrchestratorGroup,
	}
	...
	orchestratorService := orchestrator.NewService(dbOrm, publisher)
	router.AddNoPublisherHandler("sagaorchestrator_start_transaction_handler", constant.BookingTopic, subscriber, orchestratorService.StartBookingTransaction)
	router.AddNoPublisherHandler("sagaorchestrator_handle_reply_handler", constant.ReplyTopic, subscriber, orchestratorService.HandleBookingReply)
