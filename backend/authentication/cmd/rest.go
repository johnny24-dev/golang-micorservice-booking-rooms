...
var restCmd = &cobra.Command{
	Use:   "rest",
	Short: "authentication service serve rest command",
	Run:   runRestCommand,
}
...
func initializeGatewayService(ctx context.Context, gw *runtime.ServeMux, endpoint string, opts []grpc.DialOption) {
	authGwErr := pb.RegisterAuthenticationServiceHandlerFromEndpoint(ctx, gw, endpoint, opts)
	if nil != authGwErr {
		fmt.Println(authGwErr)
		os.Exit(1)
	}
}
