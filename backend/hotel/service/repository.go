...
func (r *Repository) ListAll(req *pb.ListServiceRequest) ([]*model.Service, int64, error) {
	...
	listQuery := r.db.Model(&model.Service{}).Select("*").Limit(limit).Offset(offset).Where("hotel_id = ?", req.HotelId)
	countQuery := r.db.Model(&model.Service{}).Select("id").Where("hotel_id = ?", req.HotelId)
