...
var grpcCmd = &cobra.Command{
	Use:   "grpc",
	Short: "payment service serve grpc command",
	Run:   runGrpcCommand,
}
...
func runGrpcCommand(cmd *cobra.Command, args []string) {
	...
	logrus.WithFields(logrus.Fields{
		"service": "payment",
		"type":    "grpc",
	}).Info("payment service server started")
	...
	logrus.WithFields(logrus.Fields{
		...
		"type":    "grpc",
	}).Info("payment service gracefully shutdowns")
...
func initializeServices(orm *gorm.DB, grpcServer *grpc.Server, publisher *kafka.Publisher) *grpc.Server {
	companyService := payment.NewService(orm, publisher)
	pb.RegisterPaymentServiceServer(grpcServer, companyService)
